  ...
  group('HomeBloc Reactivity', () {
    ...
    blocTest<HomeBloc, HomeState>(
      ...
      act: (bloc) async {
        bloc.add(const HomeInitialized());
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const HomeToggleFavorite('1')); // Tesla
        await Future<void>.delayed(const Duration(milliseconds: 10));
        bloc.add(const HomeToggleFavorite('3')); // Audi
      },
      ...
      expect: () => [
        predicate<HomeState>((state) {
          ...
          return favoriteItems.length == 2 &&
              favoriteItems.any((item) => item.name.contains('Tesla')) &&
              favoriteItems.any((item) => item.name.contains('Audi'));
        }),
      ],
    );
  });
